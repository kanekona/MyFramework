...
Transform::Transform(const Vec2& pos, const Vec2& s)
{
	position = pos;
	scale = s;
	angle = 0.0f;
}
Transform::Transform(const Vec2& pos, const Vec2& s,const float a)
{
	position = pos;
	scale = s;
	angle = a;
...
float KL::inner(const Vec2& _v1,const Vec2& _v2) {
...
float KL::cross(const Vec2& _v1, const Vec2& _v2) {
...
float KL::doubleinner(const Vec2& _v) {
	return inner(_v, _v);
}
...
bool KL::innerJudge(const Vec2* line, const Vec2* point) 
{
	for (int i = 0; i < 4; ++i) {
		for (int j = 0; j < 4; ++j)
		{
